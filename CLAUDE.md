...
**Visualizações YouTube (TrueView) — `metrics.video_views` foi REMOVIDO na API v22+** (`UNRECOGNIZED_FIELD` em v22/v24, confirmado via probe testando versões). Ainda existe e retorna o número real na **v21**. Solução: `gaql()` aceita um parâmetro `version` (default `ADS_VERSION` = v24); a query isolada de views (com `.catch`, só campanhas VIDEO) usa `VIEWS_VERSION = 'v21'` para pegar `metrics.video_views` real. As demais queries seguem na v24. **Atenção:** incluir `metrics.video_views` na query principal (v24) derruba TODAS as contas — por isso fica isolado. Quando o Google desativar a v21, achar o campo renomeado na versão vigente (o `.catch` faz cair pra 0 até lá). Coluna "VISUALIZAÇÕES" na tabela do Google e KPI no PDF mostram o número real.
